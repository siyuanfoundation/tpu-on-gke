...
TPU_ACCELERATOR_NAME="${TPU_ACCELERATOR_NAME:-tpu-v6e}"
...
# Fetch zones with TPU and GPU accelerator types from catalog and filter by US regions
TARGET_REGIONS="us-central1|us-east1|asia-northeast1|us-east7|southamerica-west1"
readarray -t RAW_TPU_ZONES < <(gcloud compute accelerator-types list "${PROJECT_FLAG[@]}" --filter="name='$TPU_ACCELERATOR_NAME'" --format="value(zone)" 2>/dev/null | grep -E "$TARGET_REGIONS" | sort -u)
readarray -t RAW_GPU_ZONES < <(gcloud compute accelerator-types list "${PROJECT_FLAG[@]}" --filter="name='$GPU_ACCELERATOR_NAME'" --format="value(zone)" 2>/dev/null | grep -E "$TARGET_REGIONS" | sort -u)
