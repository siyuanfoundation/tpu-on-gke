...
done < <(gcloud compute accelerator-types list "${PROJECT_FLAG[@]}" --filter="name='$TPU_ACCELERATOR_NAME'" --format="value(zone)" 2>/dev/null | grep -E "$TARGET_REGIONS" | sort -u)

RAW_GPU_ZONES=()
while IFS= read -r line; do
  [[ -n "$line" ]] && RAW_GPU_ZONES+=("$line")
done < <(gcloud compute accelerator-types list "${PROJECT_FLAG[@]}" --filter="name='$GPU_ACCELERATOR_NAME'" --format="value(zone)" 2>/dev/null | grep -E "$TARGET_REGIONS" | sort -u)
